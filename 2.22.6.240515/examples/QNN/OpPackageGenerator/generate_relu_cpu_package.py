...
# since we are working with an cpu package, we can set the backend field
...
# Users should note that the package names must match if the generated package will be used in tandem
# with an HTP package + custom model for quantization.
example_package_cpu = make_package_info(name='ReluCpu', root=os.getcwd(),
                                        backend='CPU')

# now that we have the package object, we can start creating our operators
# For this example, we will create the Relu operator for CPU
...
example_package_manager.add_package_info(example_package_cpu)
...
# (name='ReluCpu', root=os.getcwd(), backend='CPU', \
