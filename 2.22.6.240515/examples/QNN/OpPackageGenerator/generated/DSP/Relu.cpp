...
static inline float quantize(float val, float minval, float maxval) {
...
void workerThreadReluQuant(void *perOpInfrastructure, void *userData) {
  ...
  float offsetIn     = (float)quantize(0.0f, inputMin, inputMax);
  ...
  float offsetOut = (float)quantize(0.0f, outputMin, outputMax);

  for (size_t i = 0; i < inputsLen; i++) {
    float in  = (input[i] + offsetIn) * deltaIn;
    float out = in > 0.0 ? in : 0.0;
    output[i] = roundf((out / deltaOut) - offsetOut);
  }
}
