...
public class ModelCatalogueFragmentController extends
    ...
    protected void onViewAttached(final ModelCatalogueFragment view) {
        ...
        contentResolver.registerContentObserver(FileProvider.getFileFromUri(Uri.withAppendedPath(
            Model.MODELS_URI, Model.INVALID_ID)), false, mModelExtractionFailedObserver);

        contentResolver.registerContentObserver(FileProvider.getFileFromUri(Model.MODELS_URI),
            true, mModelExtractionObserver);
