   ...
   for(auto& name : outputBufferNames)
   {
       for(size_t i=0; i<batchSize; i++) {
           ...
           if (isTfNBuffer)
           {
               ...
               TfNToFloat(reinterpret_cast<float *>(&output[0]), applicationOutputBuffers.at(name).data(),ubetfN.getStepExactly0(),
                           ubetfN.getQuantizedStepSize(),applicationOutputBuffers.at(name).size() / elementSize, bitWidth);
