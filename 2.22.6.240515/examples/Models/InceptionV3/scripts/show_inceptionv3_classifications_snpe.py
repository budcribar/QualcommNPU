...
def main():
    ...
    if len(input_files) <= 0:
        ...
        result_file_name = 'Reshape_1:0.raw'
        result_file_name_win = result_file_name.replace(':', '_')
        if os.path.isfile(os.path.join(output_dir, 'Result_0/InceptionV3/Predictions', result_file_name_win)):
            result_file_name = result_file_name_win

        for idx, val in enumerate(input_files):
            cur_results_dir = 'Result_' + str(idx)
            cur_results_file = os.path.join(output_dir, cur_results_dir, 'InceptionV3/Predictions', result_file_name)
