...
from lib.device.helpers.nd_device_utilities import execute, Timeouts
from lib.utils.nd_exceptions import DeviceError
...
class Adb(object):
    def __init__(self, adb_executable, device, master_id=None):
        ...
        if len(devices) == 0:
            ...
            logger.warning('No input deviceID or wrong input deviceID, select the first device id in adb devices list: {}.'.format(self._adb_device))
