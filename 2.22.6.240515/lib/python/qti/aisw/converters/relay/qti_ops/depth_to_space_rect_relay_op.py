...
class DepthToSpaceRectRelayOp:
    _op_name = "qti.aisw.depth_to_space_rect"
    _instance = None
    ...
    def __new__(cls):
        ...
        if not cls._instance:
            ...
            # Custom D2S op with mode attributes and rectangular block_size
            custom_op = _op.get(cls._op_name)
            custom_op.set_num_inputs(1)
            custom_op.add_type_rel(cls._op_name, DepthToSpaceRectRelayOp.relation_func)
    ...
    def convert_layout(attrs, inputs, tinfos, desired_layouts):
        # update the layout for new D2S op
        new_attrs = {
        ...
        return relay.Call(_op.get(DepthToSpaceRectRelayOp._op_name), inputs, call_attrs)
