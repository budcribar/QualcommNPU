...
from qti.aisw.converters.common.utils.converter_utils import uniques, log_info, log_debug, log_warning, log_error
...
from qti.aisw.converters.tensorflow.tf_model_runtime import TensorflowModelRuntime
...
class TFConverterFrontend(ConverterFrontend):
    class ArgParser(ConverterFrontend.ArgParser):
        def __init__(self, **kwargs):
            ...
            self.add_optional_argument('-n', '--no_optimization', action='store_true', default=False,
    ...
    def __init__(self, args, *, custom_op_factory=None, validator=None):
        ...
        self._no_optimization = args.no_optimization
        ...
        self.loader = ModelLoader(args, custom_op_factory=custom_op_factory)

        if self.validator:
            (in_names, in_dims) = list(zip(*args.input_dim))
            org_tf_model = TensorflowModelRuntime(
                args.input_network,
                in_names,
                in_dims,
                args.out_names,
            ...
            self.validator.add_runtime_sessions("Original TF Model", org_tf_model)
    ...
    def convert(self):
        ...
        :rtype: IROpGraph
        """
        # FIXME: Some models are failing due to shape inference and optimizations
        #        at net run level but working fine at the converter level.
        # self.loader.utils.native_shape_inference()
        # self.loader.utils.optimize(skip_optimization=self._no_optimization)
        ...
        if self.loader.has_custom_op:
            self._graph_helper.op_collection = self.loader.custom_op_factory.op_collection
        self._convert_input_layers()
        self._convert_layers()
        self.graph.eval_macs_params()

        if self.validator:
            opt_tf_model = TensorflowModelRuntime(self.loader.model)
            self.validator.add_runtime_sessions("Optimized TF Model", opt_tf_model)
