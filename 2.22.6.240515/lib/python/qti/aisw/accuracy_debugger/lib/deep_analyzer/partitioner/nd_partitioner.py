...
class PartitionedModel:
    ...
    A PartitionedModel object contains the necessary information to
    define a subgraph within a model. A PartitionedModel object allows
    for deterministic dissection of a model. A PartitionedModel's
    tensors are always defined using Golden tensor names.
    """

    def __init__(self, input_tensors, output_tensors, verifier_type, original_accuracy=None,
...
class Partitioner:
    ...
    def __init__(self, verifier, summary_df, graph_struct, golden_tensor_paths,
                 golden_to_inference_mapping, inference_to_golden_mapping, framework, logger):
    ...
    def get_extended_inputs(self, input_tensor_names, output_tensor_names):
        ...
        Extends inputs to a previous layer while avoiding input
        conflicts, such that additional nodes are added to the
        PartitionedModel for systematic analysis. Returns tuple of
        arrays of input and output tensors names, respectively, for
        original tensor names and TensorDAG.
        ...
        while input_tensor_names:
            ...
            for parent in parents:
                for child in self.dag[parent]["children"]:  # (i)
                    if any([
                            TensorDAG.isDescendant(self.dag, output, child)
            ...
            if all_relevant_children.issubset(input_tensor_names):  # (iii)
                ...
                valid_parents = [
                    p for p in parents if p in self.inference_to_golden_mapping
                    and self.inference_to_golden_mapping[p] in self.golden_tensor_paths
                ]
                new_inputs.update(valid_parents)

                input_tensor_names.update(
                    set(parents) -
                    set(valid_parents))  # (vi) need to iterate further on invalid parents
                input_tensor_names.difference_update(
                    all_relevant_children
                )  # remove children whose parents are already accounted for
            else:
                new_inputs.add(t)  # (iv)
    ...
    def partition_analyze(self, problem_tensor_name=None, extend_inputs=False,
                          partitioned_model=None, auto_stop=False, verifier_threshold=None):
        # type: (str, bool, PartitionedModel, bool, float) -> Union[PartitionedModel, None]
        """Determine initial or subsequent PartitionedModel for use in Model
        Dissection Analysis.

        Uses VerifierAnalyzers and summary dataframe of original model
        to determine the ideal location for dissection. If a previous
        Partitioned Model is given, determine the next Partitioned Model
        to be used. Returns a new Partitioned Model object.

        :param problem_tensor_name: manually specified problematic
            inference tensor name
        :param extend_inputs: boolean flag to show inputs of old
            PartitionedModel should be extended
        :param partitioned_model: latest PartitionedModel analyzed
        :param verifier_threshold: threshold as float value to use in
            VerifierAnalyzer
        :param auto_stop: boolean flag to automatically prevent new
            PartitionedModel from being created if error in existing
            PartitionedModel is considered significant
        ...
        if not extend_inputs:
            ...
            if problem_tensor_name:
                ...
                metric_val = VerifierAnalyzer.get_metric(self.verifier, self.summary_df,
                                                         problem_tensor_name)
            else:
                # find problematic tensor
                problem_tensor_name, metric_val = VerifierAnalyzer.analyze(
                    self.verifier, self.summary_df, threshold=verifier_threshold)
            ...
            latest_inputs = [
                self.golden_to_inference_mapping[tensor_name]
                for tensor_name, _ in partitioned_model.input_tensors
            ]
            latest_outputs = [
                self.golden_to_inference_mapping[tensor_name]
                for tensor_name, _ in partitioned_model.output_tensors
            ]
            metric_val = partitioned_model.original_accuracy

            actual_accuracy = partitioned_model.actual_accuracy
            if auto_stop and VerifierAnalyzer.is_change_significant(self.verifier, self.summary_df,
                                                                    metric_val, actual_accuracy):
                return None
        inputs, outputs = self.get_extended_inputs(latest_inputs, latest_outputs)

        try:
            inputs = [[self.inference_to_golden_mapping[name], self.dim_dict[name]]
                      for name in inputs]
            outputs = [[self.inference_to_golden_mapping[name], self.dim_dict[name]]
                       for name in outputs]
        except KeyError:
    ...
    def create_partitioned_model(self, partition):
        ...
        if self.framework == Framework.tensorflow.value:
            tensorflow_suffix = ":0"
        inputs = [[input + tensorflow_suffix, dim]
                  for input, dim in zip(partition['inputs'], partition['inputs_dims'])]
    ...
    def get_relevant_nodes_from_inputs(graph_struct, golden_to_inference_mapping, input_tensors):
        ...
        :param graph_struct: GraphStructure object showing structure of
            model
        :param golden_to_inference_mapping: dict mapping golden tensor
            names to inference tensor names
        :param input_tensors: list of input tensor names
        :return: list of node names
        """

        inference_inputs = set(
            [golden_to_inference_mapping[tensor_name] for tensor_name in input_tensors])
        node_names = []
