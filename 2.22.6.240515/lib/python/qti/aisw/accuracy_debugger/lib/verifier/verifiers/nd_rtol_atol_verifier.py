...
class RtolAtolVerifier(BaseVerifier):
    ...
    def verify(self, layer_type, tensor_dimensions, golden_output, inference_output, save=True):
        """Calculates the percentage of the number of elements that are not
        close. Golden_output and inference_output must be of the same length.

        :param golden_output: List containing a single 1-D array of
            values
        :param inference_output: List containing a single 1-D array of
            values
        ...
        golden_output = golden_output[0] if isinstance(golden_output[0], np.ndarray) else np.array(
            golden_output[0])
        inference_output = inference_output[0] if isinstance(
            inference_output[0], np.ndarray) else np.array(inference_output[0])

        if len(golden_output) != len(inference_output):
            raise VerifierError(
                get_message("ERROR_VERIFIER_RTOL_ATOL_DIFFERENT_SIZE")(str(
                    len(golden_output)), (str(len(inference_output)))))
        ...
        return VerifyResult(layer_type, len(golden_output), tensor_dimensions,
                            self._get_verifier_name(), percent_not_close, self.UNITS)
    ...
    def _setup_dataframe(self, inf_size, gold_size, inf_min, gold_min, inf_max, gold_max, inf_range,
                         gold_range, error, elements_not_close, elements_over_5_error):
        self._result_dataframe = pd.DataFrame(columns=[
            RtolAtolVerifier.ERROR, RtolAtolVerifier.ELEMENTS_NOT_CLOSE,
            RtolAtolVerifier.ELEMENTS_5_ERROR, RtolAtolVerifier.INF_ARRAY_SIZE,
            RtolAtolVerifier.GOLD_ARRAY_SIZE, RtolAtolVerifier.INF_MIN_VAL,
            RtolAtolVerifier.GOLD_MIN_VAL, RtolAtolVerifier.INF_MAX_VAL,
            RtolAtolVerifier.GOLD_MAX_VAL, RtolAtolVerifier.INF_RANGE, RtolAtolVerifier.GOLD_RANGE
        ])
    ...
    def save_data(self, path):
        ...
        filename = Path(path)
        save_to_file(self._result_dataframe, filename)

    def _save_to_histogram(self, path):
        path_to_save = Path(path).with_suffix('.png')
        Visualizers.histogram_visualizer(self._inference_tensor, self._golden_tensor, path_to_save)
                ...
                if configs[i] == 'rtolmargin' or configs[i] == 'atolmargin':
                    if configs[i] not in params:
                        try:
                            ...
                            return False, {
                                'error': "Can't convert data:{}.{}".format(configs[i + 1], err_str)
                            }
                else:
