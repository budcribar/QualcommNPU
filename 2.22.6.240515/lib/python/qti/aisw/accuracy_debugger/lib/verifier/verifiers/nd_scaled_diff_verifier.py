...
class ScaledDiffVerifier(BaseVerifier):
    ...
    def verify(self, layer_type, tensor_dimensions, golden_output, inference_output, save=True):
        ...
        self._setup_dataframe(scaled_diff=scaled_diff, inf_size=len(inference_raw),
                              gold_size=len(golden_raw), inf_min=min(inference_raw),
                              gold_min=min(golden_raw), inf_max=max(inference_raw),
                              gold_max=max(golden_raw), inf_mean=np.mean(inference_raw),
    ...
    def _setup_dataframe(self, scaled_diff, inf_size, gold_size, inf_min, gold_min, inf_max,
        ...
        self._result_dataframe = pd.DataFrame(columns=[
            ScaledDiffVerifier.SCALEDDIFF, ScaledDiffVerifier.INF_ARRAY_SIZE, ScaledDiffVerifier.
            GOLD_ARRAY_SIZE, ScaledDiffVerifier.INF_MIN_VAL, ScaledDiffVerifier.GOLD_MIN_VAL,
            ScaledDiffVerifier.INF_MAX_VAL, ScaledDiffVerifier.GOLD_MAX_VAL,
            ScaledDiffVerifier.INF_MEAN, ScaledDiffVerifier.GOLD_MEAN
        ])
