...
class MeanIOUVerifier(BaseVerifier):
    ...
    def compute_iou(box1, box2):
        ...
        """Compute the IOU for two boxes. 'Boxes' are lists of size 4, where
        indices (3,2) represent the top-right corner and (1, 0) represent the
    ...
    def process_boxes(self, g_boxes_data, g_classes_data, i_boxes_data, i_classes_data):
        ...
        def join_golden_and_inference_dfs(golden_data_frame, inference_data_frame):
            ...
            """Merge a a golden DataFrame and an inference DataFrame
            horizontally by classifications. If there are more classification
            for either the golden or inference, then default to NaNs.

            ie.        IOU  boxes_golden  classification_golden
            boxes_inference  classification_inference     0  NaN  [0, 0,
            2, 2]          1               [0, 0, 3, 4]               1
            1  NaN  [0, 0, 2, 2]          1                   nan
            nan

            :param golden_data_frame: DataFrame, formatted per
                  MeanIOUVerifier.raw_data_to_data_frame
            :param inference_data_frame: DataFrame, formatted per
                  MeanIOUVerifier.raw_data_to_data_frame
            :return: a DataFrame(columns=['IOU' 'boxes_golden',
                  'boxes_inference', 'classification_golden',
            ...
            df = pd.DataFrame(columns=[
                self.GOLDEN_BOX, self.GOLDEN_CLASS, self.INF_BOX, self.INF_CLASS, self.IOU
            ])
            for classification in classes:
                ...
                inferences = inference_df[inference_df[self.CLASS] == classification].reset_index(
                    drop=True)

                combined = pd.merge(goldens, inferences, left_index=True, right_index=True,
                                    how='outer', suffixes=['_golden', '_inference'])
    ...
    def verify(self, layer_type, tensor_dimensions, golden, inference, save=True):
        ...
        if not has_valid_shapes(g_boxes, g_classes, i_boxes, i_classes):
            raise VerifierError(
                get_message('ERROR_VERIFIER_MEAN_IOU_DIFFERENT_SIZE')(len(g_boxes), len(g_classes),
                                                                      len(i_boxes), len(i_classes)))
        ...
        return VerifyResult(layer_type, g_boxes.shape, tensor_dimensions, MeanIOUVerifier.TYPE,
                            mean_iou, MeanIOUVerifier.UNITS)
