...
class SQNRVerifier(BaseVerifier):
    ...
    def verify(self, layer_type, tensor_dimensions, golden_output, inference_output, save=True):
        ...
        if len(golden_raw) != len(inference_raw):
            raise VerifierError(
                get_message("ERROR_VERIFIER_SQNR_DIFFERENT_SIZE")(str(len(golden_raw)),
        ...
        self._setup_dataframe(sqnr=sqnr, inf_size=len(inference_raw), gold_size=len(golden_raw),
                              inf_min=min(inference_raw), gold_min=min(golden_raw),
                              inf_max=max(inference_raw), gold_max=max(golden_raw),
                              inf_mean=np.mean(inference_raw), gold_mean=np.mean(golden_raw))
        return VerifyResult(layer_type, len(golden_raw), tensor_dimensions, SQNRVerifier.NAME, sqnr,
                            SQNRVerifier.UNITS)

    def _setup_dataframe(self, sqnr, inf_size, gold_size, inf_min, gold_min, inf_max, gold_max,
                         inf_mean, gold_mean):
        self._result_dataframe = pd.DataFrame(columns=[
            SQNRVerifier.SQNR, SQNRVerifier.INF_ARRAY_SIZE, SQNRVerifier.GOLD_ARRAY_SIZE,
            SQNRVerifier.INF_MIN_VAL, SQNRVerifier.GOLD_MIN_VAL, SQNRVerifier.INF_MAX_VAL,
            SQNRVerifier.GOLD_MAX_VAL, SQNRVerifier.INF_MEAN, SQNRVerifier.GOLD_MEAN
        ])
