...
class MSEVerifier(BaseVerifier):
    ...
    def verify(self, layer_type, tensor_dimensions, golden_output, inference_output, save=True):
        ...
        if len(golden_raw) != len(inference_raw):
            raise VerifierError(
                get_message("ERROR_VERIFIER_MSE_DIFFERENT_SIZE")(str(len(golden_raw)),
        ...
        if not save:
            ...
            return match, mse
        self._setup_dataframe(mse=mse, delta=delta, inf_size=len(inference_raw),
    ...
    def _setup_dataframe(self, mse, delta, inf_size, gold_size, inf_min, gold_min, inf_max,
        ...
        self._result_dataframe = pd.DataFrame(columns=[
            MSEVerifier.MSE, MSEVerifier.DELTA, MSEVerifier.INF_ARRAY_SIZE, MSEVerifier.
            GOLD_ARRAY_SIZE, MSEVerifier.INF_MIN_VAL, MSEVerifier.GOLD_MIN_VAL, MSEVerifier.
            INF_MAX_VAL, MSEVerifier.GOLD_MAX_VAL, MSEVerifier.INF_MEAN, MSEVerifier.GOLD_MEAN
        ])
