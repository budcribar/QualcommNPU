...
class L1ErrorVerifier(BaseVerifier):
    ...
    def verify(self, layer_type, tensor_dimensions, golden_output, inference_output, save=True):
        ...
        if len(golden_raw) != len(inference_raw):
            raise VerifierError(
                get_message("ERROR_VERIFIER_L1ERROR_DIFFERENT_SIZE")(str(len(golden_raw)),
                                                                     (str(len(inference_raw)))))
        ...
        if self.mean:
            return VerifyResult(layer_type, len(golden_raw), tensor_dimensions,
                                L1ErrorVerifier.MAE_NAME, l1_mean, L1ErrorVerifier.MAE_UNITS)
        else:
            return VerifyResult(layer_type, len(golden_raw), tensor_dimensions,
                                L1ErrorVerifier.L1_NAME, l1_error, L1ErrorVerifier.L1_UNITS)

    def _setup_dataframe(self, error, sum, mean, delta, inf_size, gold_size, inf_min, gold_min,
                         inf_max, gold_max, inf_range, gold_range):
        self._result_dataframe = pd.DataFrame(columns=[
            L1ErrorVerifier.ERROR, L1ErrorVerifier.SUM, L1ErrorVerifier.MEAN,
            L1ErrorVerifier.MULTIPLIER, L1ErrorVerifier.SCALE, L1ErrorVerifier.DELTA,
            L1ErrorVerifier.INF_ARRAY_SIZE, L1ErrorVerifier.GOLD_ARRAY_SIZE,
            L1ErrorVerifier.INF_MIN_VAL, L1ErrorVerifier.GOLD_MIN_VAL, L1ErrorVerifier.INF_MAX_VAL,
            L1ErrorVerifier.GOLD_MAX_VAL, L1ErrorVerifier.INF_RANGE, L1ErrorVerifier.GOLD_RANGE
        ])
