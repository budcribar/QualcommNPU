...
from qti.aisw.accuracy_debugger.lib.utils.nd_constants import Architecture_Target_Types, Engine, Android_Architectures, X86_Architectures, X86_windows_Architectures
...
class WrapperCmdOptions(CmdOptions):
    ...
    def initialize(self):
        ...
        optional.add_argument(
            '--engine_version', type=str, required=False,
    ...
    def verify_update_parsed_args(self, parsed_args):
        ...
        if parsed_args.engine is not None:
            if len(parsed_args.engine) > 2:
                ...
                if parsed_args.engine_version is None:
