...
class TensorDAG(defaultdict):
    ...
    def __init__(self, graph_struct, available_tensors):
        ...
        value is nested dictionary. With regard to inner dictionary,
        keys are 'parents' and 'children', and value is set of tensor
        names.

        E.g. For a node with input A and outputs B,C: dag = {
        'tensorA': {         'parents': {},         'children':
        {'tensorB','tensorC'}     },     'tensorB': {         'parents':
        {'tensorA'},         'children': {}     },     'tensorC': {
        'parents': {'tensorA'},         'children':{}     } }

        :param graph_struct: GraphStructure object showing structure of
            model
        :param available_tensors: list of available inference tensor
            names
        ...
        for component in graph_struct._components:
            ...
            for output_tensor in component.output_tensors.keys():
                if output_tensor in available_tensors:
                    self[output_tensor]["parents"].update(
                        [t for t in component.input_tensors.keys() if t in available_tensors])
            for input_tensor in component.input_tensors.keys():
                if input_tensor in available_tensors:
                    self[input_tensor]["children"].update(
                        [t for t in component.output_tensors.keys() if t in available_tensors])
    ...
    def getLCA(dag, tensor_list):
        ...
        """Returns lowest common ancestor given TensorDAG and iterable of
        tensor names.
