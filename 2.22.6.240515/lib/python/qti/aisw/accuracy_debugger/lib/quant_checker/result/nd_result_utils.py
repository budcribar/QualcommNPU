...
import re
from .nd_result_node import ResultNode
        ...
        for resultNode in results[quantizationVariation]:
            ...
            for comparisonAlgorithmName, comparisonResult in resultNode.getResults().items():
                if comparisonResult != dict():
                    comparisonPassFails[
                        comparisonAlgorithmName] = translateTrueFalseToPassFailForHtml(
                            comparisonResult['pass'])
            resultList.append(
                ResultNode(resultNode.getNodeName(), resultNode.getTensorName(),
                           resultNode.getTensorType(), comparisonPassFails,
                           str(resultNode.getScale()), str(resultNode.getOffset())))
    return resultList
...
def getActivationResultsForHtml(quantizationVariation, inputFile, results) -> List[ResultNode]:
    ...
    if quantizationVariation != 'unquantized' and quantizationVariation in results.keys():
        ...
        for activationResultNode in perInputResults:
            comparisonPassFails = {}
            for comparisonAlgorithmName, comparisonResult in activationResultNode.getResults(
            ).items():
                if comparisonResult != dict():
                    comparisonPassFails[
                        comparisonAlgorithmName] = translateTrueFalseToPassFailForHtml(
                            comparisonResult['pass'])
            resultList.append(
                ResultNode(activationResultNode.getNodeName(), activationResultNode.getTensorName(),
                           activationResultNode.getTensorType(), comparisonPassFails,
                           str(activationResultNode.getScale()),
                           str(activationResultNode.getOffset())))
    return resultList
...
def getInputResultsForHtml(results) -> List[ResultNode]:
    ...
    for resultNode in results.values():
        ...
        for comparisonAlgorithmName, comparisonResult in resultNode.getResults().items():
            if comparisonResult != dict():
                comparisonPassFails[comparisonAlgorithmName] = translateTrueFalseToPassFailForHtml(
                    comparisonResult['pass'])
        resultList.append(
            ResultNode(resultNode.getNodeName(), resultNode.getTensorName(),
                       resultNode.getTensorName(), comparisonPassFails))
    return resultList
...
def getAnalysisDescription(results) -> str:
    return Verifiers.getFailureAnalysisDescription(
        getListOfAlgorithmNamesThatFailedFromHtmlResults(results))
...
def getListOfResultsWithoutAlgorithms(results) -> List:
    ...
    for result in results:
        noVerifiers.append([
            result.getNodeName(),
            result.getTensorName(),
            result.getTensorType(),
            str(result.getScale()),
            str(result.getOffset())
        ])
    return noVerifiers
...
def formatResultsForLogConsole(quantizationVariations, results,
                               showOnlyFailedResults=False) -> Dict[str, List]:
    formattedResults = {}
    for quantizationVariation in quantizationVariations:
        ...
        for resultNode in resultList:
            for comparisonAlgorithmName, comparisonResult in resultNode.getResults().items():
                if comparisonResult != dict():
                    ...
                    quantOptionResults.append([
                        resultNode.getNodeName(),
                        resultNode.getTensorName(), comparisonResult['pass'],
                        comparisonResult['data'], comparisonResult['threshold'],
                        comparisonAlgorithmName,
                        str(resultNode.getScale()),
                        str(resultNode.getOffset())
                    ])
...
def formatActivationResultsForLogConsole(quantizationVariations, results,
                                         showOnlyFailedResults=False) -> Dict[str, List]:
    logFormattedResults = {}
        ...
        for activationResultNode in resultList:
            for comparisonAlgorithmName, comparisonResult in activationResultNode.getResults(
                ...
                if comparisonResult != dict():
                    ...
                    quantOptionResults.append([
                        activationResultNode.getNodeName(),
                        activationResultNode.getTensorName(), comparisonResult['pass'],
                        comparisonResult['data'], comparisonResult['threshold'],
                        comparisonAlgorithmName,
                        activationResultNode.getInputFilename(),
                        str(activationResultNode.getScale()),
                        str(activationResultNode.getOffset())
                    ])
        if len(quantOptionResults) != 0:
...
def formatActivationsForCsv(resultList) -> Tuple:
    ...
    for activationResultNode in resultList:
        formatDataForCsv(activationResultNode.getNodeName(), activationResultNode.getResults(),
                         quantOptionResults, quantOptionResultsHeader,
                         activationResultNode.getTensorName(), None, None,
                         activationResultNode.getInputFilename())
...
def formatWeightsAndBiasesForCsv(resultList) -> Tuple:
    ...
    quantOptionResultsHeader = {}
    for resultNode in resultList:
        formatDataForCsv(resultNode.getNodeName(), resultNode.getResults(), quantOptionResults,
                         quantOptionResultsHeader, resultNode.getTensorName(),
                         str(resultNode.getScale()), str(resultNode.getOffset()))
    return (quantOptionResults, quantOptionResultsHeader)
...
def formatInputsForCsv(results) -> Tuple:
    ...
    for filename, resultNode in results.items():
        formatDataForCsv(filename, resultNode.getResults(), quantOptionResults,
                         quantOptionResultsHeader, 'input')
    return (quantOptionResults, quantOptionResultsHeader)


def formatDataForCsv(opName, comparisonResults, quantOptionResults, quantOptionResultsHeader,
                     nodeName=None, scale=None, offset=None, inputFilename=None):
    for comparisonAlgorithmName, comparisonResult in comparisonResults.items():
        if comparisonResult != dict():
            ...
            resultData = re.findall(r'[-+]?(?:(?:\d*\.\d+)|(?:\d+\.?))(?:[Ee][+-]?\d+)?',
                                    str(comparisonResult['data']))
            resultHeader = re.findall(r'[a-zA-Z ]+[a-zA-Z]+[^ 0-9Ee]',
                                      str(comparisonResult['data']))
            if scale and offset:
...
def getFailurePercentage(quantizationVariations, failureNodeCount, totalNodeCount):
    ...
    for quantizationVariation in quantizationVariations:
        ...
        if totalNodeCountForQuantization == 'N/A':
            ...
            quantizationFailurePercentage[quantizationVariation] = (
                float(failureNodeCountForQuantization) /
                float(totalNodeCountForQuantization)) * 100.0
...
def getFailureNodeCount(verifierName, quantizationVariations, results):
    ...
    for quantizationVariation in quantizationVariations:
        resultList = results[quantizationVariation]
        if resultList:
            verifierFailureCount[quantizationVariation] = resultList.getFailureCountForVerifier(
                verifierName)
