            ...
            for _, _, file_list in outputs:
                for file_path in file_list:
                    ...
                    if self.engine == Engine.QNN.value:
                        tensor_mapping[
                            tensor_name] = santize_node_name(self.framework_instance.get_mapping_for_qnn_node(
                                tensor_name))
                    elif self.engine == Engine.SNPE.value:
                        tensor_mapping[
                            tensor_name] = self.framework_instance.get_mapping_for_snpe_node(
                                tensor_name)
            ...
            for _, _, file_list in outputs:
                for file_path in file_list:
                    ...
                    if tensor_name in tensor_mapping:
                        continue
                    if self.engine == Engine.QNN.value:
                        tensor_mapping[tensor_name] = self.get_mapping_for_qnn_node_with_golden_dir(
                            tensor_name)
                    elif self.engine == Engine.SNPE.value:
                        tensor_mapping[
