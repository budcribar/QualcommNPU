...
class SNPEConverter(Converter):
    ...
    def __init__(self, context):
        ...
        self.quantization_overrides_flag = context.arguments["quantization_overrides_flag"]
    ...
    def build_convert_command(self, model_path, input_tensors, output_tensors, output_path,
                              quantization_overrides, extra_converter_args=None):
        ...
        if len(model_paths) != len(self.model_path_flags):
            raise InferenceEngineError(
                get_message("ERROR_INFERENCE_ENGINE_MISMATCH_MODEL_PATH_INPUTS"))
        ...
        if self.input_tensor_flag:
            for input_tensor, dimension in formatted_input_tensors.items():
                convert_command_list.extend(
                    [self.input_tensor_flag, '"{}"'.format(input_tensor), dimension])
        ...
        if quantization_overrides:
            convert_command_list.extend([self.quantization_overrides_flag, quantization_overrides])
