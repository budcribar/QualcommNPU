...
class SNPEInferenceEngine(InferenceEngine):
    ...
    def __init__(self, context, converter, executor):
        ...
        arch = 'aarch64-windows-msvc' if self.host_device.device in ['wos'] else self.host_device.device
        self._CONVERTER_LOCATION = context.converter_location.format(
            engine_path=self.engine_path, host_arch=arch)
    ...
    def _set_base_dlc(self):
        ...
        rows, total_params, total_macs, total_memory_cpu, total_memory_fxp, total_op_types = m.extract_model_info(
            graph_names[0])
    ...
    def _execute_inference(self, dlc_data):
        ...
        if self.target_device.device in ["x86", "x86_64-windows-msvc", "wos"]:
            self.host_env['LD_LIBRARY_PATH'] = os.path.join(self.engine_path, "lib",
                                                            arch)
        else:
