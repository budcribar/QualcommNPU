...
class TensorFlowFramework_1_15_0(BaseFramework):
    __VERSION__ = '1.15.0'

    def __init__(self, logger):
        super(TensorFlowFramework_1_15_0, self).__init__(logger)
        self._graph = None
        ...
        with tf.Session(graph=self._graph) as sess:
            for data, tensor_name in zip(input_data, input_tensor_names):
                ...
                if tensor.get_shape().dims is not None:
                    for tensor_dimension, data_dimension in zip(tensor.get_shape().as_list(),
                                                                data.shape):
                        if tensor_dimension != data_dimension and tensor_dimension is not None:
                            raise FrameworkError(
                                get_message("ERROR_FRAMEWORK_TENSORFLOW_MISMATCH_INPUT_DIMENSIONS"))
                else:
                    self.logger.warning(
                        get_warning_message("WARNING_FRAMEWORK_TENSORFLOW_DIMENSION_UNSPECIFIED"))

            input_dict = {
                name: data
                for name, data in zip(input_tensor_names, input_data)
                if self._graph.is_feedable(self._graph.get_tensor_by_name(name))
            }

            output_tensors = [
                sess.graph.get_tensor_by_name(tensor_name) for tensor_name in output_tensor_names
            ]
            ...
            for output_tensor in output_tensors:
                try:
                    ...
                    logging.debug(
                        get_debugging_message("DEBUG_FRAMEWORK_TENSORFLOW_TENSOR_NOT_EVALUATED")(
                            str(exc)))
                except tf.errors.OpError as exc:
                    logging.warning(
                        get_warning_message("WARNING_FRAMEWORK_TENSORFLOW_TENSOR_NOT_EVALUATED")(
                            str(exc)))
    ...
    def get_intermediate_tensors(self, input_tensors, output_tensors):
        input_ops = [
            self._graph.get_tensor_by_name(tensor_name).op for tensor_name in input_tensors
        ]
        output_ops = [
            self._graph.get_tensor_by_name(tensor_name).op for tensor_name in output_tensors
        ]

        ops = tf.contrib.graph_editor.get_walks_intersection_ops(input_ops, output_ops,
    ...
    def filter_blocks(self, tensor_pairs, block):
        ...
        for scope in scope_to_remove:
            tensor_pairs = TensorFlowFramework_1_15_0.filter_block(tensor_pairs, scope)
    ...
    def filter_block(tensor_pairs, remove_scope):
        ...
        for input_tensors, output_tensors in tensor_pairs:
            block_inputs = [
                tensor_name for tensor_name in input_tensors
                if any(remove_scope in tensor
                       for tensor in output_tensors) and remove_scope not in tensor_name
            ]
            block_outputs = [
                tensor_name for tensor_name in input_tensors
                if any(remove_scope not in tensor
                       for tensor in output_tensors) and remove_scope in tensor_name
            ]

            inputs_to_map_block = inputs_to_map_block + list(
                set(block_inputs) - set(inputs_to_map_block))
            outputs_to_map_block = outputs_to_map_block + list(
                set(block_outputs) - set(outputs_to_map_block))

        tensor_pairs = list(
            filter((lambda pair: not any(remove_scope in name
                                         for name in pair[0]) or not any(remove_scope in name
                                                                         for name in pair[1])),
                   tensor_pairs))
    ...
    def get_graph_structure(self):
        ...
        for op in self._graph.get_operations():
            input_dict = {
                input_tensor.name: self.get_dimensions(input_tensor.name)
                for input_tensor in op.inputs
            }
            output_dict = {
                output_tensor.name: self.get_dimensions(output_tensor.name)
                for output_tensor in op.outputs
            }
    ...
    def _find_act_quant_node(self, node_name):
        for tensor in tf.compat.v1.get_default_graph().as_graph_def().node:
            if len(
                    tensor.input
            ) > 0 and tensor.input[0] == node_name and tensor.op == "FakeQuantWithMinMaxVars":
                return tensor.name
        return node_name
    ...
    def get_mapping_for_qnn_node(self, qnn_output):
        ...
        self.logger.warning(
            get_warning_message("WARNING_FRAMEWORK_TENSORFLOW_MISMATCH_TENSOR")(qnn_output))
        return " "
