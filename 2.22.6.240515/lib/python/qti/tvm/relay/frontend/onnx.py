...
### [QTI Change]
class LpNormalization(OnnxOpConverter):
    """A helper class for lpnormalization op converters."""
    ...
    def _impl_v1(cls, inputs, attr, params):
        ...
        if p != 2:
            raise ValueError("Only the L2-Norm is supported. "
                             "Found order of {}".format(p))

        return _op.nn.l2_normalize(x, eps=1e-12, axis=[axis])
### [End QTI Change]
...
class Pad(OnnxOpConverter):
    ...
    ### [QTI Change]
    # add default layout for Onnx pad op
    data_layout_dict = {
        2: "NF",
        3: "NCW",
        4: "NCHW",
    ...
    def _impl_v1(cls, inputs, attr, params):
        ...
        ### [QTI Change]
        # add default layout for Onnx pad op
        attr["data_layout"] = cls.data_layout_dict[dims]
        ### [End QTI Change]
    ...
    def _impl_v11(cls, inputs, attr, params):
        ...
        data_layout = cls.data_layout_dict[len(input_shape)]
        return _op.nn.pad(inputs[0], pad_width_expr, value, pad_mode=pad_mode, data_layout=data_layout)
        ### [End QTI Change]
...
class Prelu(OnnxOpConverter):
    ...
    def _impl_v1(cls, inputs, attr, params):
        ...
        alpha_shape = infer_shape(alpha)
        if len(data_shape) != len(alpha_shape):
            # Expand alpha's dims to the data's last dims to make alpha broadcastable
            alpha = _op.expand_dims(inputs[1], 0, len(data_shape)-len(alpha_shape))
...
class Resize(OnnxOpConverter):
    ...
    def v11_13_common(cls, inputs, size, attr, params):
        ...
        if mode == "nearest":
            method = "nearest_neighbor"
        ### [QTI Change]
        elif mode == "linear" or mode in ["bilinear", "trilinear"]: # some models violate onnx spec using "bi" or "tri" prefix
            method = "linear"
        ### [End QTI Change]
        elif mode == "cubic":
            method = "cubic"
...
def _get_convert_map(opset):
    return {
        ...
        ### [QTI Change]
        'LpNormalization': LpNormalization.get_converter(opset),
        ### [End QTI Change]
...
class GraphProto:
    ...
    def _construct_nodes(self, graph):
        ...
        for node in graph.node:
            ...
            ### [QTI Change]
            op = set_span(op, span=node_source_name, op_type=node.op_type, output_names=node_output)
            ### [End QTI Change]
        ...
        for i in node.input:
            if i != "":
                ...
                if isinstance(expr, (relay.Var, relay.Constant)):
                    ### [QTI Change]
                    expr_with_span = set_span(expr, span=node.name, output_names=[name])
                    ### [End QTI Change]
