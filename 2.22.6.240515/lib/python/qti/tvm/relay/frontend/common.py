...
class ExprTable(object):
    ...
    ### [QTI Change]
    def new_const(self, value, shape=None, dtype="float32", source_name='', output_names=[], output_qparams=[]):
        ...
        self.exprs[name] = set_span(self.exprs[name], source_name, output_names=output_names,
...
class _SpanFiller(ExprMutator):
    ...
    def __init__(self, span, op_type, output_names, output_qparams):
        """
        Parameters
        ----------
        span : str, relay.Span or relay.SequentialSpan
        op_type : str
        output_names : list[str]
        output_qparams : list[dict{str, str/tvm.nd.array(float)}]
        """
        span = span.decode("utf-8") if isinstance(span, bytes) else span
        output_names = [output_name.decode("utf-8") if isinstance(output_name, bytes) else output_name for output_name in output_names]
        output_qparams_dict = dict()
        for output_name, output_qparam in zip(output_names, output_qparams):
            if output_qparam != None:
                output_qparams_dict[output_name] = \
                    tvm.relay.SourceQparams(output_qparam["dtype"], output_qparam["scale"], output_qparam["zero_point"])
        ExprMutator.__init__(self)
        if isinstance(span, (tvm.relay.Span, tvm.relay.SequentialSpan)):
            self._span = span
        elif isinstance(span, str):
            self._span = tvm.relay.Span(tvm.relay.SourceName(span), 0, 0, 0, 0, op_type, output_names, output_qparams_dict)
        else:
            assert False, f"unsupported span type: {type(span)}"

        """
        only set output name for output node (distance == 0).
        for intermediate node (distance != 0), we don't set buffer name in span
        """
        self.distance = -1
        if isinstance(span, tvm.relay.SequentialSpan):
            span_name = self._span.spans[-1].source_name.name
            self._span_for_output_expr = self._span
            self._span_for_intermediate_expr = tvm.relay.Span(tvm.relay.SourceName(span_name), 0, 0, 0, 0, op_type)
        else:
            span_name = self._span.source_name.name
            self._span_for_output_expr = self._span
            self._span_for_intermediate_expr = tvm.relay.Span(tvm.relay.SourceName(span_name), 0, 0, 0, 0, op_type)

    def reset_span(self):
        if self.distance == 0:
            self._span = self._span_for_output_expr
        else:
            self._span = self._span_for_intermediate_expr
...
### [QTI Change]
def set_span(sym, span='', op_type='', output_names=[], output_qparams=[]):
    """
    ...
    op_type : String
        The source information of the op type.

    output_names : List[String]
        The source information of the output buffer names

    output_qparams : List[Dict[String, String/NDArray]]
        The source information of the output buffer quantization parameters. Include "dtype" saved
        in String and "scale" and "zero_point" saved in NDArray.
    ...
    .. code-block:: python
      ...
      x = set_span(
        relay.var("x", shape=(1,64,56,56)), "x_var", output_names=["input"],
        output_qparams=[{"scale": tvm.nd.array(np.ones(1, np.float32)), "zero_point": tvm.nd.array(np.zeros(1, np.int32)), "dtype": "int8"}]
      )
      w = set_span(
        relay.const(np.ones([64,64,3,3]), dtype="int8"), output_names=["weight"],
        output_qparams=[{"scale": tvm.nd.array(np.ones(64, np.float32)), "zero_point": tvm.nd.array(np.zeros(64, np.int32)), "dtype": "int8"}]
      )
      y = set_span(
        relay.nn.conv2d(x, w, channels=64, kernel_size=(3, 3), padding=(1, 1)), "conv2d", output_names=["output"],
        output_qparams=[{"scale": tvm.nd.array(np.ones(1, np.float32)), "zero_point": tvm.nd.array(np.zeros(1, np.int32)), "dtype": "int8"}]
      )
      print(relay.Function([x], y))

      #fn (%x: Tensor[(1, 64, 56, 56), float32] /* span=x_var:0:0, output_names:["input"], output_qparams:{"input": Qparams{scale: 1f, zero_point: 0, dtype: int8}} */) {
      #  nn.conv2d(%x, meta[relay.Constant][0] /* span=:0:0, output_names:["weight"], output_qparams:{"weight": Qparams{scale: NDArray[(64), float32], zero_point: NDArray[(64), int32], dtype: int8}} */,
      #            padding=[1, 1, 1, 1], channels=64, kernel_size=[3, 3]) /* span=conv2d:0:0, output_names:["output"], output_qparams:{"output": Qparams{scale: 1f, zero_point: 0, dtype: int8}} */
      #}
    """

    if tvm.transform.PassContext.current().config.get("relay.frontend.fill_span", True):
        return _SpanFiller(span, op_type, output_names, output_qparams).fill(sym)
    return sym
### [End QTI Change]


### [QTI Change]
def get_enclosed_nodes_as_list(ordered_node_dict, input_tensor_list=[], output_node_list=[]):
    """
    Traverse the nodes in order and return the node name list of partial graph which
    is enclosed by starting points and end points.

    Parameters
    ----------
    ordered_node_dict : OrderedDict of str to dict
        OrderedDict of node name to a dict storing "inputs" and "outputs" as keys

    input_tensor_list : List of strings
        Graph level input tensor name list
        The specified tensor names can be any tensor within the model, which
        determines where the returned node_name_list starts from.

    output_node_list : List of strings
        Graph level output node name list
        The specified node names can be node at any location within the model, which
        determines the endpoints of the graph traversal.
        The rest of unvisited nodes will not be traversed once all the specified
        output nodes have been visited.
        If the list is not given, the graph traversal will continue to the end.

    Returns
    -------
    node_name_list : List of strings
        The node name list required in the partial graph
    """
    class InvalidPartialGraphError(Exception):
        def __init__(self, message):
            super().__init__(message)

    def detect_if_multiple_partial_graphs(tracked_node_map):
        detected_graph_list = []
        for node_info in tracked_node_map.values():
            input_name_list = node_info.get("inputs")
            output_name_list = node_info.get("outputs")
            first_detected_graph = None
            for graph in detected_graph_list:
                if any([input_name in graph for input_name in input_name_list]):
                    if first_detected_graph:
                        # Merge the set if currently visited input exists in multiple graphs
                        first_detected_graph.update(graph)
                        graph.clear()
                    else:
                        graph.update(input_name_list)
                        graph.update(output_name_list)
                        first_detected_graph = graph

            if not first_detected_graph:
                detected_graph_list.append({*input_name_list, *output_name_list})

        num_partial_graph = len([graph for graph in detected_graph_list if graph])
        if num_partial_graph > 1:
            raise InvalidPartialGraphError("Only 1 partial graph is applicable to be specified, found {} graphs".format(
                num_partial_graph))

    def backtrack_prune_redundant_node(tracked_node_map, node_name_list):
        """
        Prune redundant nodes that are not necessary on the path from starting point to end point by
        traversing the node map in reversed order.
        """
        backtracked_edges = set()
        for node_name, node_info in reversed(tracked_node_map.items()):
            if node_name in output_node_list or any([edge in backtracked_edges for edge in node_info.get("outputs")]):
                backtracked_edges.update(node_info.get("inputs"))
            else:
                node_name_list.remove(node_name)

    tracked_node_map = OrderedDict()
    input_is_specified = bool(input_tensor_list)
    output_is_specified = bool(output_node_list)
    tracked_edges = set(input_tensor_list)
    unvisited_output_node_list = output_node_list[:]

    for node_name, node_info in ordered_node_dict.items():
        if not input_is_specified:
            tracked_node_map[node_name] = node_info
        else:
            edges_in_track = [edge in tracked_edges for edge in node_info.get("inputs")]
            if all(edges_in_track):
                if any([name in input_tensor_list for name in node_info.get("outputs")]):
                    raise InvalidPartialGraphError("The output of specified node `{}` can not be assigned as graph input".format(
                        node_name))
                tracked_node_map[node_name] = node_info
                tracked_edges.update(node_info.get("outputs"))
            elif any(edges_in_track):
                raise InvalidPartialGraphError("Node `{}` requires all inputs `{}` to be specified".format(
                    node_name, node_info.get("inputs")))

        if node_name in unvisited_output_node_list:
            unvisited_output_node_list.remove(node_name)
        if output_is_specified and not unvisited_output_node_list:
            # Early exit if all output nodes are visited
            break

    if unvisited_output_node_list:
        raise InvalidPartialGraphError("The graph requires output nodes `{}` to be visited, found unvisited node(s) `{}`".format(
            output_node_list, unvisited_output_node_list))
    if not tracked_node_map:
        raise InvalidPartialGraphError("Output node is visited before user-specified input(s)")

    node_name_list = list(tracked_node_map.keys())
    detect_if_multiple_partial_graphs(tracked_node_map)
    if output_is_specified:
        backtrack_prune_redundant_node(tracked_node_map, node_name_list)
