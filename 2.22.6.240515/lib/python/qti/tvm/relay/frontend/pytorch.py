...
class PyTorchOpConverter:
    ...
    def arange(self, inputs, input_types):
        def _get_value(val, dtype):
            ...
            if isinstance(val, _expr.Expr):
                ...
                node_source_name = self._get_op_name_for_span(op_node)
                inp = set_span(_op.cast(val, dtype), node_source_name, op_type=op_node.kind())
                ### [End QTI Change]
    ...
    def pad_common(self, mode, pad_value, inputs, input_types):
        ### [QTI Change]
        # add default layout for PyTorch pad op
        data_layout_dict = {
            2: "NF",
            3: "NCW",
            4: "NCHW",
        ...
        ### [QTI Change]
        # add default layout for PyTorch pad op
        data_layout = data_layout_dict[int(pad_len / 2)]
        ### [End QTI Change]
        ...
        if not non_zero_found:
            ...
            ### [QTI Change]
            return _op.nn.pad(data, const_paddings, pad_value=pad_value, pad_mode=mode, data_layout=data_layout)
            ### [End QTI Change]
        else:
            ### [QTI Change]
            return _op.nn.pad(data, const_paddings, pad_mode=mode, data_layout=data_layout)
            ### [End QTI Change]
    ...
    ### [QTI Change] ###
    def make_upsample1d(self, method):
        def upsample1d(inputs, input_types):
            data = inputs[0]
            ...
            return _op.image.resize1d(data, out_size, None, "NCW", method, coord_trans)
    ...
    def nms(self, inputs, input_types):
        ...
        op_node = self.current_op[-1]
        node_source_name = self._get_op_name_for_span(op_node)
        scores = set_span(scores - _op.min(scores) + _op.const(1.0), node_source_name, op_type=op_node.kind())

        num_boxes = set_span(_op.shape_of(scores), node_source_name, op_type=op_node.kind())
        # PyTorch NMS doesn't have score_threshold, so no need to run get_valid_count
        # - since "arange" op will fill expr into its attribute
        # - invoke set_span here to prevent expr-rewritten occurrs in span-filling stage
        indices = _op.transform.arange(set_span(_op.squeeze(num_boxes), node_source_name, op_type=op_node.kind()), dtype="int32")
        ### [End QTI Change]
    ...
    ### [QTI Change]
    def fake_quantize_per_tensor_affine(self, inputs, input_types):
        # https://pytorch.org/docs/stable/generated/torch.fake_quantize_per_tensor_affine.html
        quant_min = inputs[3]
        quant_max = inputs[4]
        bitwidth = int(np.log2(quant_max-quant_min+1))
        if quant_min < 0:
            dtype='int'
        else:
            dtype='uint'
        out_dtype = dtype + str(bitwidth)
    ...
    def channel_shuffle(self, inputs, input_types):
        data = inputs[0]
        group = inputs[1]

        data_shape = list(self.infer_shape(data))
        if data_shape[1] % int(group) != 0:
            raise ValueError("Input channel: {} is not divisible by group: {}".format(data_shape[1], int(group)))

        new_data_shape = data_shape[:1] + [int(group), data_shape[1]//int(group)] + data_shape[2:]

        # [N, C, H, W] -> [N, g, C/g, H, W] -> [N, C/g, g, H, W] -> [N, C, H, W]
        reshape = _op.reshape(data, new_data_shape)
        transpose = _op.transpose(reshape, axes=(0, 2, 1, 3, 4))
        return _op.reshape(transpose, data_shape)
    ### [End QTI Change]
    ...
    def create_convert_map(self):
        self.convert_map = {
            ...
            ### [QTI Change] ###
            "aten::upsample_linear1d": self.make_upsample1d("linear"),
            ### [End QTI Change] ###
            ...
            ### [QTI Change]
            "aten::fake_quantize_per_tensor_affine": self.fake_quantize_per_tensor_affine,
            "aten::channel_shuffle": self.channel_shuffle,
            ### [End QTI Change]
        }
    ...
    ### [QTI Change]
    def report_missing_conversion(self, op_names, custom_op_factory):
        from qti.aisw.converters.pytorch.pytorch_to_ir import get_valid_type_name, get_domain_and_valid_type_name
    ### [End QTI Change]
        ...
        if custom_op_factory and len(custom_op_factory.op_collection):
            known_ops += [key for key in custom_op_factory.op_collection.__dict__.keys() \
                          if key != 'op_count']
        ...
        for op_name in op_names:
            ...
            if op_name not in known_ops and \
                not (op_name.endswith("_") and op_name[:-1] in known_ops) and \
                get_valid_type_name(op_name) not in known_ops and \
                not (get_domain_and_valid_type_name(op_name)[0] == "qti_aisw"):
    ...
    def convert_loop(self, loop_node, outputs):
        ...
        ### [QTI Change]
        node_source_name = self.source_map[loop_node]
        loop_iter_var = set_span(
            _expr.var(block_input_names[0], shape=(), dtype=loop_iter_dtype), span=node_source_name
        )
        loop_vars = set_span(
            [get_var(name, val) for name, val in name_val_pairs[1:]], span=node_source_name
        )
        ### [End QTI Change]
        ...
        for name in free_vars:
            ...
            ### [QTI Change]
            outputs[name] = set_span(new_loop_var, node_source_name)
            ### [End QTI Change]
    ...
    ### [QTI Change]
    def convert_custom(self, custom_op_type, inputs, output_names, node_source_name, custom_ops_dict,
                       domain_name=None):
        from qti.aisw.converters.backend.custom_ops.core import qnn_to_dtype
        from qti.aisw.converters.common.converter_ir.axis_tracker import PytorchAxisOrder
        ...
        def custom_op_rel(arg_types, attrs):
            def valid_output_dimension(op):
                valid_output_dim = True
                for out_dim in op.output_dims:
                    if not any(out_dim):
                        # if the output dimension is [] or [0], set valid_output_dim to False, and
                        # then the output shape needs to be inferred using shape inference library later
                        valid_output_dim = False
                return valid_output_dim

            if domain_name == "qti_aisw":
                qti_aisw_op = create_qti_aisw_op(custom_op_type, **attrs)
                # input_axis_formats not used for shape inference, setting to NONTRIVIAL
                input_shapes, input_axis_formats = [], []
                for i in range(len(arg_types)):
                    input_shapes.append(arg_types[i].shape)
                    input_axis_formats.append("NONTRIVIAL")
                output_dims = qti_aisw_op.infer_shape(input_shapes, input_axis_formats, len(output_names), PytorchAxisOrder())
                output_type = [tvm.relay.TensorType(output_shape, output.dtype)
                               for output_shape, output in zip(output_dims, arg_types)]
            ...
            # We use a dictionary (aka. custom_ops_dict) to store custom pytorch ops in the op collection
            # of the op factory, and the desired op can be obtained by the node source name of custom call.
            custom_pytorch_op = custom_ops_dict[str(attrs['node_source_name'])]
            # If the output dim is already recorded in the custom pytorch op, we can retrieve it directly.
            if valid_output_dimension(custom_pytorch_op):
                output_dims = custom_pytorch_op.output_dims
            # If not, the output shape is inferred using the shape inference library
            # linked by custom op in op adapter.
            else:
                qnn_custom_op = create_custom_op(custom_pytorch_op)
                input_shapes, input_axis_formats = [], []
                for i, input_info in enumerate(custom_pytorch_op.inputs):
                    input_shapes.append(list(arg_types[i].concrete_shape))
                    input_axis_formats.append(custom_pytorch_op.axis_orders[input_info.name])
                output_dims = qnn_custom_op.infer_shape(input_shapes, input_axis_formats,
                                                        len(output_names), PytorchAxisOrder())

                # Set output shape of the custom pytorch op in the op collection of custom op factory
                custom_pytorch_op.output_dims = output_dims

            output_type = [tvm.relay.TensorType(output_dim, qnn_to_dtype(output.data_type))
                           for output_dim, output in zip(output_dims, custom_pytorch_op.outputs)]

            return tvm.relay.TupleType(output_type)

        # register custom call
        try:
            _op.op.register(custom_op_type)
            _op.op.register_pattern(custom_op_type, _op.OpPattern.OPAQUE)
            _op.op.register_stateful(custom_op_type, False)
        except tvm.error.TVMError as e:
            pass

        new_inputs = [inp for inp in inputs if isinstance(inp, _expr.Expr)]
        custom_op = _op.op.get(custom_op_type)
        custom_op.set_num_inputs(len(new_inputs))
        custom_op.set_attrs_type_key("DictAttrs")
        custom_op.add_type_rel(custom_op_type, custom_op_rel)
        custom_op.set_support_level(1)
        # Set attribute "node_source_name" to record the node_source_name of custom call
        new_attrs = {'node_source_name': node_source_name}
    ...
    def convert_operators(self, operators, outputs, ret_names, custom_op_factory=None):
        """Convert each Torch IR operators to Relay equivalent"""
        ### [QTI Change]
        from qti.aisw.converters.pytorch.pytorch_to_ir import get_valid_type_name, get_domain_and_valid_type_name
        from qti.aisw.converters.relay.importers.relay_importer import RelaySpanParser
        # custom_ops_dict: a dictionary for recording node source name and custom pytorch op
        #   - key: node source name which is the same as span.source_name.name
        #   - value: custom pytorch op which is stored in the op collection of op factory
        custom_ops_dict = dict()
        span_parser = RelaySpanParser(dict())
        ### [End QTI Change]
        for node_name, op_node in operators:
            ...
            domain_name, op_type = get_domain_and_valid_type_name(operator)
            if (custom_op_factory and op_type in custom_op_factory.op_collection) or domain_name == "qti_aisw":
                node_source_name = self._get_op_name_for_span(op_node)
                node_source_name = get_valid_type_name(node_source_name)
                output_names = _get_output_names(op_node)
                if domain_name == "qti_aisw":
                    pass
                else:
                    custom_ops_dict[node_source_name] = custom_op_factory.op_collection.get_op_by_name(
                        op_type, span_parser.get_op_name(node_source_name, op_type))
                custom_out = self.convert_custom(op_type, inputs, output_names,
                                                 node_source_name, custom_ops_dict,
                                                 domain_name)
                custom_out = set_span(custom_out, node_source_name, op_type=op_node.kind())
                self.record_output_type(custom_out)
                outputs.update(zip(output_names, custom_out))
            ### [End QTI Change]
                ...
                outputs.update(
                    zip(
                        ...
                        set_span(unpacked, node_source_name, op_type=op_node.kind())
                    )
                )
                ### [End QTI Change]
                ...
                outputs.update(
                    zip(
                        unpacked_names,
                        set_span(loop_out, node_source_name, op_type=op_node.kind())
                    )
                )
                ### [End QTI Change]
            else:
                ...
                node_source_name = self._get_op_name_for_span(op_node)
                relay_out = set_span(relay_out, node_source_name, op_type=op_node.kind())
                ### [End QTI Change]
    ...
    ### [QTI Change]
    def _get_op_name_for_span(self, op_node):
        torch_node_name = self._get_node_name(op_node)
        node_source_name = torch_node_name if torch_node_name else self.source_map[op_node]
        return node_source_name
    ### [End QTI Change]
    ...
    def _set_parameter_source_name(self, op_node, outputs):
        ...
        for name in _get_input_names(op_node):
            ...
            if isinstance(expr, (_expr.Var, _expr.Constant)):
                ...
                node_source_name = [self.source_map[op_node]]
                if isinstance(expr, _expr.Var):
                    ...
                    if expr.name_hint.startswith(node_source_name[0]):
                        node_source_name[0] = expr.name_hint.split(name_sep)[-1]
                    else:
                        node_source_name.append(expr.name_hint.split(name_sep)[-1])

                if isinstance(expr, _expr.Var):
                    new_expr = set_span(expr, name_sep.join(node_source_name), op_type=op_node.kind(), output_names=[expr.name_hint])
                else:
                    new_expr = set_span(expr, name_sep.join(node_source_name), op_type=op_node.kind())
...
def _pytorch_result_type(dtypes, non_tensor_inputs):
    ...
    for inp in non_tensor_inputs:
        ### [QTI Change]
        # Cast Python Int class (int64) to torch.int to correspond with constant datatype conversion
        cast_inp = torch.tensor(inp, dtype=torch.int) if isinstance(inp, int) else inp
        result_type = _convert_data_type(
            str(torch.result_type(torch.zeros((), dtype=dtype_map[result_type]), cast_inp))
        )
        ### [End QTI Change]
    return result_type
...
def _rename_outputs(node, source_map, op_type_dict, use_parser_friendly_name):
    ...
    if node.kind() != "prim::GetAttr":
        ...
        ### [QTI Change]
        # force to sanitize 'aten::' to 'aten__' here
...
def _get_relay_input_vars(graph, input_infos, prelude, is_module=True, default_dtype="float32"):
    ...
    for ir_input, (name, itype) in zip(ir_inputs, input_types):
        ...
        ### [QTI Change]
        inp = set_span(inp, span=name, output_names=[name])
        ### [End QTI Change]
...
### [End QTI Change]
    ...
    for node in getattr_nodes:
        ...
        for getattrs in get_attr_chains(node):
            ...
            ### [QTI Change]
            # _getattr_full_name will connect all attr name
            # e.g., given torch IR
            ...
            #     return (%6)
            # the return value for %4 node is "model.fc.weight"
            full_attr = _getattr_full_name(getattrs, attr_name_sep)
            full_attr_node_name = _get_output_name(getattrs[-1])
            # the following lines will change source params name, we comment out to keep their source name
            # check change in https://github.com/apache/tvm/pull/14050/files#diff-a9664111082fead669fd3e5a1d894d22838452e180aaf93ac45b598b2de7211fR4672
            ###############################################################################
            # set variable name by concatenating first consumer's name with full attribute
            # e.g. var_name = "".join(["aten::batch_norm_5", "running_mean"] = aten::batch_norm_5.running_mean
...
def from_pytorch(
    ...
    ### [QTI Change]
    custom_op_factory : CustomOpFactory object
        Custom op factory stores custom pytorch op whose information is extracted from the xml config.
    ### [End QTI Change]
    ...
    ### [QTI Change]
    converter.report_missing_conversion(op_names, custom_op_factory)
    ### [End QTI Change]
    ...
    ### [QTI Change]
    outputs = converter.convert_operators(operator_nodes, outputs, ret_name, custom_op_factory)
    ### [End QTI Change]
