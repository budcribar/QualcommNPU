...
class Adb(object):
    ...
    @retry()
    def shell(self, command, args=[], output_to_list=True):
        if output_to_list:
        ...
        code, out, err = self._execute('shell', shell_args, output_to_list=output_to_list)
        if output_to_list:
            if code == 0:
                if len(out) > 0:
                    try:
    ...
    def _execute(self, command, args, cwd='.', recovery_enable=True, output_to_list=True):
        adb_command_args = [
            '-H',
            self._hostname,
            '-s',
        ...
        (return_code, output, error) = execute(self.__adb_executable,
                                               adb_command_args, cwd=cwd, timeout=Timeouts.ADB_DEFAULT_TIMEOUT, output_to_list=output_to_list)
