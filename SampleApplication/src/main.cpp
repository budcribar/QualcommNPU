...
std::unique_ptr<sample_app::QnnSampleApp> processCommandLine(int argc,
  ...
  static struct pal::Option s_longOptions[] = {
      ...
      {"duration", pal::required_argument, NULL, OPT_DURATION_SECONDS},
      {NULL, 0, NULL, 0}};
  ...
  while ((opt = pal::getOptLongOnly(argc, argv, "", s_longOptions, &longIndex)) != -1) {
    switch (opt) {
      ...
      case OPT_DURATION_SECONDS:
          ...
          try {
              seconds = std::stoi(pal::g_optArg);
          }
          catch (const std::invalid_argument& e) {
              std::cerr << "Invalid argument: " << pal::g_optArg << " is not a valid number." << std::endl;
              std::exit(EXIT_FAILURE);
          }
          catch (const std::out_of_range& e) {
              std::cerr << "Out of range: " << pal::g_optArg << " is too large." << std::endl;
              std::exit(EXIT_FAILURE);
          }
...
int main(int argc, char** argv) {
...

  {
    ...
    std::unique_ptr<sample_app::QnnSampleApp> app =
        sample_app::processCommandLine(argc, argv, loadFromCachedBinary, seconds);
    ...
    if (sample_app::StatusCode::SUCCESS != app->executeGraphs(seconds)) {
